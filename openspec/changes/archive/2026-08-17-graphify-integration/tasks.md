...
- [x] **T-06** `internal/status/display_test.go`: add `TestDisplay_Graphify` mirroring `TestDisplay_Impeccable` (line 139) — two sub-tests: (a) disabled: output contains `"Graphify (Code Graph)"` and `"Enabled:   false"` but not the version/dir strings; (b) enabled with version `"v0.9.45"` and output_dir `".archon/graphify"`: output contains both strings. **R-03**
