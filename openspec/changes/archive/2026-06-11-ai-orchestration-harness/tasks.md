...
- [x] 1.1 Init Go module, add deps (cobra v1.8, yaml.v3) — `go.mod`, `go.sum`
- [x] 1.2 `internal/config/config.go` — Config struct, Load/Save with injectable HomeDir, YAML roundtrip
- [x] 1.3 `internal/config/rollback.go` — RollbackManifest, WriteManifest, Cleanup, backup/restore AGENTS.md
- [x] 1.4 `internal/version/info.go` — ldflags: Version, Commit, Date; Print()
- [x] 1.5 `skills/embed.go` — `//go:embed */SKILL.md` with 21 existing skills
- [x] 1.6 Tests: config Load/Save (MapFS), rollback manifest lifecycle, version output
...
- [x] 3.1 `internal/scaffold/extract.go` — fs.ReadDir → MkdirAll → WriteFile for all 21 skills
- [x] 3.2 `internal/scaffold/symlink.go` — SymlinkOrCopy with copy fallback on EPERM/EINVAL
- [x] 3.3 `internal/init/templates.go` — embed AGENTS.md/CLAUDE.md orchestrator template
- [x] 3.4 `internal/init/init.go` — orchestrator: detect→extract→symlink→config→template chain
- [x] 3.5 Version-gap detection — compare metadata.version frontmatter, prompt on mismatch
- [x] 3.6 Tests: extract idempotency, symlink fallback, init orchestration flow, version-gap prompt
