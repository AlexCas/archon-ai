Feature: Graphify Integration (Slice A — advisory code-graph gate)
  ...
  @happy
  Scenario: CLAUDE.md contains group G question, mapping paragraph, seven-group count
    When the orchestrator renders CLAUDE.md
    Then the file contains "¿Activar Graphify para análisis de grafo de código?"
    And the file contains "Group G maps to" and "graphify.enabled" and "--graphify"
    And the group count reads "seven" and the range reads "A–G"
    And the recommended option for group G is "No"
