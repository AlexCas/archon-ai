...
- Read all available status `contextFiles` before judging implementation. Full spec-driven verification reads proposal, specs (including Gherkin `.feature` files), design, and tasks; partial artifact sets degrade as described below.
...
- **Security coverage check (conditional)**: If `security.enabled` is true, confirm
  ...
  covering test or scanner step — as CRITICAL. When `security.enabled` is false,
